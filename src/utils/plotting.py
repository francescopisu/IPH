...
def plot_average_roc_curves(cv_preds: Dict[str, Dict[str, list]],
    ...
    for j, (algo_name, outer_cv_data) in enumerate(cv_preds.items()):
        ...
        lab = "{} ({:.3f} [{:.3f} - {:.3f}])".format(ALGO_NAMES_MAP[algo_name], auc_med, auc_low, auc_up)

        ax.plot(fpr,
                tpr,
                label=lab,
                lw=3.0 if algo_name == conf.TUNING.ALGORITHM_TO_TUNE and highlight_best else 1.1,
...
def plot_average_pr_curves(cv_preds: Dict[str, Dict[str, list]],
    ...
    for j, (algo_name, outer_cv_data) in enumerate(cv_preds.items()):
        ...
        lab = "{} ({:.3f} [{:.3f} - {:.3f}])".format(ALGO_NAMES_MAP[algo_name], pr_auc_med, pr_auc_low, pr_auc_up)
